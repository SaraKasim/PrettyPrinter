...
object Prettyprinter {
  ...
  def indentSearch(context: String, is: IndentSpec): Int = {
    ...
    is match {
      ...
      case h :: t => if (context == h._1) h._2
      else indentSearch(context, t)
  ...
  def prettyPrintCommand(command: Command, is: IndentSpec): List[String] = {
    ...
    command match {
      case Nop              => "nop" :: Nil
      case Set(Var(x), exp) => x + " := " + prettyPrintExpr(exp) :: Nil
      case While(exp, listCom) => ("while " +
        prettyPrintExpr(exp) +
        " do" ::
        (appendStringBeforeAll(makeIndent(indentSearch("WHILE", is)), prettyPrintCommands(listCom, is)))) ++
        ("od" :: Nil)
      case For(exp, listCom) => ("for " +
        prettyPrintExpr(exp) +
        " do" ::
        (appendStringBeforeAll(makeIndent(indentSearch("FOR", is)), prettyPrintCommands(listCom, is)))) ++
        ("od" :: Nil)
      case If(exp, listCom, listCom2) => ("if " +
        prettyPrintExpr(exp) +
        " then" ::
        (appendStringBeforeAll(makeIndent(indentSearch("IF", is)), prettyPrintCommands(listCom, is))) ++
        ("else" :: Nil) ++
        (appendStringBeforeAll(makeIndent(indentSearch("IF", is)), prettyPrintCommands(listCom2, is)))) ++
        ("fi" :: Nil)
    }
  ...
  def prettyPrintCommands(commands: List[Command], is: IndentSpec): List[String] = {
    commands match {
      case Nil      => Nil
      case h :: Nil => prettyPrintCommand(h, is)
      case h :: t   => appendStringAfterLast(" ;", prettyPrintCommand(h, is)) ++ prettyPrintCommands(t, is)
    }
  ...
  def prettyPrintProgram(program: Program, is: IndentSpec): List[String] = {
    program match {
      case Progr(i, b, o) => "read " +
        prettyPrintIn(i) :: "%" ::
        (appendStringBeforeAll(makeIndent(indentSearch("PROGR", is)), prettyPrintCommands(b, is))) ++
        ("%" :: ("write "
          + prettyPrintOut(o) :: Nil))
    }
  ...
  def prettyPrint(program: Program, is: IndentSpec): String = {
    var resList: List[String] = prettyPrintProgram(program, is)
    val resString = resList.mkString("\n")
    return resString
  }
